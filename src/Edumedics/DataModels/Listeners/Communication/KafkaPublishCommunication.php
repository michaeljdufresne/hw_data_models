...
namespace Edumedics\DataModels\Listeners\Communication;
...
class KafkaPublishCommunication extends ListenerAppResolver
{
    ...
    public function handle(Event $event)
    {
        if (isset($this->resolvedService) && isset($this->resolvedService->kafkaProducerQueue))
        {
            try
            {
                ...
                if ($eventType == CommunicationDelete::class)
                {
                    $eventCopy->communication = $eventCopy->communication->toArray();
                }
            ...
            {
                Log::info("Produce Communication to Kafka failed: " . $e->getMessage());
            }
        }
        else
        {
            Log::info("Unable to resolve KafkaProxyService or find Kafka Producer Queue - KafkaPublishCommunication");
        }
