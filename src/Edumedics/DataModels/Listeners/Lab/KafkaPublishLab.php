...
namespace Edumedics\DataModels\Listeners\Lab;
...
class KafkaPublishLab extends ListenerAppResolver
{
    ...
    public function handle(Event $event)
    {
        if (isset($this->resolvedService) && isset($this->resolvedService->kafkaProducerQueue))
        {
            try
            {
                ...
                if ($eventType == LabDelete::class)
                {
                    $eventCopy->lab = $eventCopy->lab->toArray();
                }
            ...
            {
                Log::info("Produce Lab to Kafka failed: " . $e->getMessage());
            }
        }
        else
        {
            Log::info("Unable to resolve KafkaProxyService or find Kafka Producer Queue - KafkaPublishLab");
        }
    }
}