...
class KafkaPublishAppointment extends ListenerAppResolver
{
    ...
    public function handle(Event $event)
    {
        if (isset($this->resolvedService) && isset($this->resolvedService->kafkaProducerQueue))
        {
            try
            {
                ...
                if ($eventType == AppointmentDelete::class)
                {
                    $eventCopy->appointment = $eventCopy->appointment->toArray();
                }
            ...
            {
                Log::info("Produce Appointment to Kafka failed: " . $e->getMessage());
            }
        }
        else
        {
            Log::info("Unable to resolve KafkaProxyService or find Kafka Producer Queue - KafkaPublishAppointment");
        }
